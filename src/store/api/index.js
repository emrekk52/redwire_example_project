import { articlesCollection, firebase, usersCollection, videosCollection } from "../../firebase";
...
export const autoSignIn = () =>
  new Promise((resolve, reject) => {
    firebase.auth().onAuthStateChanged((user) => {
      ...
      else resolve({ isAuth: false, user: [] });
    });
  });
...
export const getMoreArticles = async (articles) => {
  ...
  try {
    if (lastPostVisible) {
      ...
      return { posts: [...articles.posts,...newArticles], lastPostVisible: lastPostVisible };
    }
...
export const getMoreVideos = async (articles) => {
  ...
  try {
    if (lastVideoVisible) {
      ...
      const newVideos = response.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
      }));

      return { videos: [...articles.videos,...newVideos], lastVideoVisible: lastVideoVisible };
    }
